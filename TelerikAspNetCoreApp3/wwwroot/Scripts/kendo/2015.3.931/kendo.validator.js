            ...
            var Validator = Widget.extend({
                ...
                options: {
                    ...
                    messages: {
                        ...
                        unique: function (input) { return input.attr('data-val-singleunique'); }
                    },
                    uniqueValue: [],
                    rules: {
                        ...
                        date: function (input) {
                            if (input.filter("[type^=date],[" + kendo.attr("type") + "=date]").length && input.val() !== "") {
                                try {
                                    if ((new pDate(+(input.data('kendoDatePicker').value())).persianFormat() == 
                                        input.data('kendoDatePicker').value().persianFormat()))
                        ...
                        unique: function (input, preValue) {
                            if (typeof input.attr('data-val-singleunique') !== "undefined") {
                                var flag = false;
                                var propName = input.attr('name');

                                if (preValue != undefined)
                                    this.uniqueValue = preValue;
                                else {
                                    preValue = [];
                                    preValue[propName] = this.uniqueValue[propName];
                                }


                                //var arrays = input.val().toString().split('');
                                //$.each(arrays, function(v, i) {
                                //    if (preValue[propName][i] != v)
                                //        flag = false;
                                //});

                                var url = 'FieldExists';

                                if (preValue[propName] == input.val()
                                    || (preValue.length == 0 && this.uniqueValue[propName] == undefined)
                                    )
                                    flag = true;
                                else {
                                    $.ajax({
                                        ...
                                        async: false,
                                        dataType: "json",
                                        data: { propertyName: propName, value: input.val() },
                ...
                _checkValidity: function (input, preValue) {
                    var rules = this.options.rules,
                        rule;
                    
                    for (rule in rules) {
                        if (rule == "unique") {
                            if (!rules[rule].call(this, input, preValue)) {
                                return { valid: false, key: rule };
                            }
                        } else {
                            if (!rules[rule].call(this, input)) {
