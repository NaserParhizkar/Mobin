...
(function ($, undefined) {
    ...
    var TabStrip = Widget.extend({
        init: function(element, options) {
            ...
            that.wrapper.children(".k-tabstrip-items")
                ...
                .on(CLICK + NS, " > " + NAVIGATABLEITEMS, function (e) {
                    ...
                    if ($(e.target).hasClass('closedTab')) {
                        if (that.items().length > 1 && $(that.items()).is(':visible')) {
                            //if (($(e.currentTarget).index() + 1) < that.items().length) {
                            if ($(e.currentTarget).is("." + ACTIVESTATE)) {
                                if (!that.activeNextVisibleTab($(e.currentTarget).index() + 1)) {
                                    that.activePreviousVisibleTab($(e.currentTarget).index() - 1);
                                }
                            }
                                //if (!that.activeNextVisibleTab($(e.currentTarget).index() + 1)) {
                                //    that.activePreviousVisibleTab($(e.currentTarget).index());
                                //}
                            //}
                        }
                        //debugger;
                        //that.contentElements.splice($(e.currentTarget).index(), 1);
                        that.contentHolder($(e.currentTarget).index()).hide();
        ...
        activePreviousVisibleTab : function (tabindex){
            if ($(this.items()[tabindex]).is(':visible')) {
                this.select(tabindex);
                return true;
            }
            else if (this.items()[tabindex - 1]) {
                this.activePreviousVisibleTab(tabindex - 1);
            }
        },

        activeNextVisibleTab: function (tabindex) {
            if (this.items()[tabindex] && $(this.items()[tabindex]).is(':visible')) {
                this.select(tabindex);
                return true;
            }
            else if (this.items()[tabindex + 1]) {
                this.activeNextVisibleTab(tabindex + 1);
            }
        },
        ...
        _updateClasses: function() {
            ...
            //naser/////////////////////////////////////////////////////////////////////////////
            if(that.options.closeableFlag) 
                that.tabGroup.find("li").append('<span class="k-icon k-i-close closedTab"></span>')
            //////////////////////////////////////////////////////////////////////////////////////
