        ...
        (function ($, undefined) {
            ...
            var Window = Widget.extend({
                ...
                _overlay: function (visible) {
                    ...
                    if (!overlay.length) {
                        overlay = $("<div class='k-overlay' tabindex='100' />");
                    }
                ...
                _activate: function () {
                    var scrollable = this.options.scrollable !== false, that = this;
                    ...
                    //naser edited//////////////////////////////////////////////////////////////////////////////////
                    $('.k-widget.k-window').delegate('*', 'keydown', function (event) {
                        if (event.which == 13) {
                            if (!$('.k-button.k-button-icontext.k-grid-cancel').is(':focus')) {
                                $(event.target).closest('.k-popup-edit-form').find('.k-button.k-button-icontext.k-primary.k-grid-update').focus();
                                $('.k-button.k-button-icontext.k-primary.k-grid-update').trigger('click');
                                return false;
                            }
                        }
                            //for exit when esc pressed
                        else if (event.which == 27) {
                            if ($('.k-widget.k-window').find('.k-window-content.k-content').length > 0)
                                $('.k-widget.k-window').find('.k-window-content.k-content').focus();
                        }
                    });

                    //for exit when esc pressed
                    $('.k-overlay').bind('keydown', function (e) {
                        var editContainerForm = $(this);
                        editContainerForm = editContainerForm.siblings().find('.k-window-content.k-content');
                        if (editContainerForm.length > 0 && e.which == 27)
                            editContainerForm.focus();
                    });

                    //for set focus in first element
                    if (this.element.find('input').first().attr('data-role') == "dropdownlist"
                        || this.element.find('input').first().attr('data-role') == "autocomplete"
                        || this.element.find('input').first().attr('data-role') == "combobox") {
                        var dataRole = this.element.find('input').first().attr('data-role') == "dropdownlist" ? "DropDownList" :
                            (this.element.find('input').first().attr('data-role') == "autocomplete" ? "AutoComplete" : "ComboBox");
                        this.element.find('input').first().data("kendo" + dataRole).focus();
                ...
                _realElement: function (elem) {
                    debugger;
                    var element = $(elem);
                    if (element.attr('data-role') == "dropdownlist"
                       || element.attr('data-role') == "autocomplete"
                       || element.attr('data-role') == "combobox") {
                        var dataRole = element.attr('data-role') == "dropdownlist" ? "DropDownList" :
                            (element.attr('data-role') == "autocomplete" ? "AutoComplete" : "ComboBox");
                        return element.data("kendo" + dataRole);
                    }
                    return element;
                },
