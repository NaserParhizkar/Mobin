    ...
    var validationRules = {
        required: function (input) {
            if (input.data('kendoDatePicker')) {
                if (input.data('kendoDatePicker').options.culture == 'fa-IR') {
                    var realArrValue = input.val().split('/');
                    var formatArrValue = input.data('kendoDatePicker').options.format.split('/');
                    if ((formatArrValue[0].replace('yyyy', '----') == realArrValue[0]) &&
                        (formatArrValue[1].replace('MM', '--') == realArrValue[1]) &&
                        (formatArrValue[2].replace('dd', '--') == realArrValue[2]))
                        return false;
                }
            }
        ...
        date: function (input) {
            try {
                ...
                if (new pDate(+(input.data('kendoDatePicker').value())).persianFormat() == input.data('kendoDatePicker').value().persianFormat())
