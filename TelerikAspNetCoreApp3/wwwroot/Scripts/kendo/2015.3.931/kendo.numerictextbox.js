    ...
    (function () {
        (function ($, undefined) {
            ...
            var NumericTextBox = Widget.extend({
                    ...
                    that._focusing = setTimeout(function () {
                        ...
                        if (result) {
                            if (that.options.format == "n0" || that.options.format == "c")
                                caretPosition = result[0].replace(groupRegExp, "").length + (Math.round(value.replace(/,/g,'').length/3));
                            else
                        ...
                        //naser edited
                        var selectedLocation = that._focusin();
                        if (selectedLocation.start != -1 && selectedLocation.end != -1)
                            caret(that.element[0], selectedLocation.start, selectedLocation.end);
                        else
                ...
                _focusin: function () {
                    var that = this,
                        selectedLocation = { start: -1, end: -1 };

                    //naser edited 
                    // for set selected value in case of press tab key and select value of input
                    var element = that._text;
                    var selection = kendo.caret(element);
                    selectedLocation.start = selection[0];
                    selectedLocation.end = selection[1];
                    ////////////////////////////////////
                ...
                _keydown: function (e) {
                    ...
                    if (e.keyCode == keys.BACKSPACE || e.keyCode == keys.DELETE) {
                        ...
                        var str = element.val();
                        var deletedChar = str.charAt(selectionStart - 1);
                        if (deletedChar === ',') {
                            var value = element.val().substring(0, selectionStart-2) + element.val().substring(selectionStart , element.val().length);
                            element.val(value);
                            caret(this.element, selectionStart-2);
                            e.preventDefault();
                            return false
                        }
                    }
                ...
                _keypress: function (e) {
                    ...
                    if (that._numericRegex(numberFormat).test(value.replace(/,/g, ''))) {
                        if (that.options.format === "n0" || that.options.format === "c") {
                            ...
                            caret(this.element, selectionStart + character.length + (element.val().length - value.length));
                            e.preventDefault();
                        }
                    }
                    //////////
                ...
                _keyup: function (e) {
                    ...
                    var selectionStart = selection[0];
                    var commaDecrease = 0;
                    if ((element.val().replace(/,/g, '').length + 1) % 3 == 1)
                        commaDecrease = 1;
                    ...
                    if (e.keyCode == keys.BACKSPACE || e.keyCode == keys.DELETE) {
                        if (this._numericRegex(this._format(this.options.format)).test(this.element.val().replace(/,/g, ''))) {
                            if (this.options.format === "n0" || this.options.format === "c") {
                                ...
                                caret(this.element, selectionStart - commaDecrease);
                                e.preventDefault();
                            }
                        }
                    }
                ...
                _toggleText: function (toggle) {
                    var that = this;
                    if (that.options.format == "n0" || that.options.format == "c")
                        that.element.val(this.commaSeparatedNumber(that.element.val()));
