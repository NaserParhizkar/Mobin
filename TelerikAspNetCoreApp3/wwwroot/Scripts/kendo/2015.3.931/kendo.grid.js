...
*/
(function (f, define) {
    define(["./kendo.data", "./kendo.columnsorter", "./kendo.editable", "./kendo.window", "./kendo.filtermenu", "./kendo.columnmenu",
        "./kendo.groupable", "./kendo.pager", "./kendo.selectable", "./kendo.sortable", "./kendo.reorderable",
        "./kendo.resizable", "./kendo.mobile.actionsheet", "./kendo.mobile.pane", "./kendo.ooxml", "./kendo.excel", "./kendo.progressbar", "./kendo.pdf"], f);
})(function () {
        ...
        (function ($, undefined) {
            ...
            var Grid = kendo.ui.DataBoundWidget.extend({
                init: function (element, options, events) {
                    ...
                    var deleteConfirmWindow = '<div class="windowDeleteConfirmation k-rtl d-command-bar"></div>';
                    $(deleteConfirmWindow).appendTo('body');
                ...
                gridErrors: ["entity_Read", "entity_Insert", "entity_Update", "entity_Delete"],
                ...
                options: {
                    ...
                    messages: {
                        ...
                        commands: {
                            ...
                            insert: defaultCommands.insert.text,
                ...
                _errorNotification: function () {
                    var that = this, options = that.options;
                    var errorElement = $('<div id=""></div>').appendTo(that.wrapper);

                    var notification = new kendo.ui.Notification(errorElement);
                    if (options.errorNotification instanceof kendo.ui.Notification) {
                        notification = options.errorNotification
                    } else {
                        notification.setOptions({
                            "position": { "top": 10, "right": 5 }, "autoHideAfter": 10000,
                            "animation": { "close": { "effects": "zoom:out", "duration": 400 } },
                            "templates": [{ "type": "error", "templateId": "errorTemplate" }]
                        });
                    }
                    that.errorNotification = notification;
                },
                destroy: function () {
                    var that = this,
                        element;
                ...
                editRow: function (row) {
                    ...
                    $.ajax({
                        url: "/"+this.dataSource.options.transport.read.url.split('/')[1] + "/GetEntityByKey",
                        type: 'post',
                        data: {
                            "key": model.id
                        },
                        success: function (response) {
                            for (var v in response) {
                                var relationModel = this.dataSource.options.schema.model.id.split('.')[0];
                                var property = v;
                                var returnModel = {};
                                returnModel[relationModel] = {};
                                returnModel[relationModel][property] = {};
                                $('#' + relationModel + '_' + property).val(response[v]);
                            }
                        },
                ...
                _createPopupEditor: function (model) {
                    ...
                    if (command) {
                        if (isPlainObject(command)) {
                            if (command.text && isPlainObject(command.text)) {
                                ...
                                insertText = command.text.insert;
                            }
                    ...
                    if (!that._isMobile) {
                        ...
                        var popupWindow = $(html).appendTo(that.wrapper).eq(0);

                        container = that._editContainer =
                            popupWindow.kendoWindow(extend({
                                modal: true,
                                resizable: false,
                                draggable: true,
                                title: "Edit",
                    ...
                    for (var prop in model.fields) {
                        if (model.fields[prop].type == 'object') {
                            popupWindow.find("#" + prop +"_"+ model.fields[prop].primaryKey).val(model[prop + model.fields[prop].primaryKey]);
                        }
                    }
                    ///////////////////////////////////////////////////
                ...
                _openPopUpEditor: function () {
                    if (!this._isMobile) {
                        var popup = this._editContainer.data("kendoWindow").center().open();
                    } else {
                ...
                saveRow: function () {
                    ...
                    if (container && editable && editable.end(model) &&
                        !that.trigger(SAVE, { container: container, model: model })) {
                        //$.each(model.fields, function (i, v) {
                        //    if (v.type == 'date')
                        //        model[i] = model[i].toJSON();

                        //});
                        kendo.ui.progress(this.editable.element, true);
                        that.dataSource.sync();
                    }
                },
                ...
                _bindSelectedRow: function () {
                    var that = this,
                        item = that.dataItem(that.select());

                    if (item && item != undefined) {
                        that.options.tags.forEach(function (v, i) {
                            var relationModel = v.field.split('.')[0];
                            var property = v.field.split('.')[1];
                            var returnModel = {};
                            returnModel[relationModel] = {};
                            returnModel[relationModel][property] = {};
                            $('#' + relationModel + '_' + property).val(item[relationModel][property]);
                        });

                        if (that.options.tags.length > 0) {
                            var relatedModel = that.options.tags[0].field.split('.')[0];
                            var primaryKeyValue = item[relatedModel][item.fields[relatedModel].primaryKey];
                            //var primaryKeyProp = item.fields[relatedModel].primaryKey;
                            var fields = $("#" + that.options.masterGrid).data("kendoGrid").editable.options.model.fields;
                            for (var field in fields) {
                                if (fields[field]["foreignKeyClassName"] === relatedModel) {
                                    $("#" + that.options.masterGrid).data("kendoGrid").editable.options.model[field] = primaryKeyValue;
                                }
                            }
                            //$('#' + relatedModel + '_' + primaryKeyProp).val(primaryKeyValue);
                        }
                    }
                },
                ...
                _error: function (ev) {
                    //naser edited 
                    var that = this;
                    var errors = that.gridErrors;

                    if (ev.errors) {
                        for (var i = 0; i < errors.length; i++) {
                            if (ev.errors[errors[i]]) {
                                var notification = that.errorNotification;
                                notification.show({
                                    title: "وضیعت عملیات",
                                    message: ev.errors[errors[i]].errors[0]
                                }, "error");
                            }
                        }
                    }
                ...
                _modelChange: function (e) {
                    ...
                    if (row.add(relatedRow).children(".k-edit-cell").length && !that.options.rowTemplate) {
                        ...
                        var tempDate = null;
                        var dateFieldName = "";
                        $.each(model.fields, function (field, fieldOpt) {
                            if ($('#' + e.field).attr('data-role') == "datepicker" && fieldOpt.type == 'date'
                                && $('#' + e.field).data('kendoDatePicker').options.culture == 'fa-IR') {
                                dateFieldName = e.field;
                            }
                        })
                        tempDate = model[dateFieldName];
                        if ($('#' + e.field).attr('data-role') == "datepicker"
                                && $('#' + e.field).data('kendoDatePicker').options.culture == 'fa-IR') {
                            model[dateFieldName] = model[dateFieldName] ? model[dateFieldName].persianFormat() : model[dateFieldName];
                        }
                        tmp = (isAlt ? that.altRowTemplate : that.rowTemplate)(model);

                        model[dateFieldName] = tempDate;
                ...
                _tags: function (tags) {
                    var that = this,
                        encoded;
                    var grid = this.element.attr('id');

                    //tags.forEach(function (v, i) {
                    //    var elements = '<div>' +
                    //       '<label For="' + v.dependentModelName + "_" + v.field + '" id="' + "lbl" + v.dependentModelName + "_" + v.field + '">' + v.title +':</label>' +
                    //       '<input type="text" class="k-textbox" id="' + v.dependentModelName + "_" + v.field + '"/ >' +
                    //       '</div>';
                    //    $('#' + grid).prepend(elements);
                    //});
